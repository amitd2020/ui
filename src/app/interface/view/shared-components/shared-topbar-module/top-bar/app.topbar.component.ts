...
@Component({
    ...
    styleUrls: [ '../../../../../layout/utilities/dg-utilities-overrides.component.scss', 'app.topbar.component.scss' ],
	animations: [
    ...
    ngOnInit() {
        this.userAuthService.isLoggedInSubject$.subscribe( ( loggedIn: boolean ) => {
			...
			if ( this.isLoggedIn ) {
				...
				if ( [ '/', '/user-account-info/user-subscription', '/user-account-info/user-profile', '/hubspot-ui/dg-authenticate', '/hubspot-ui/update', '/hubspot-ui/filters' ].includes( this.router.url ) ) {
