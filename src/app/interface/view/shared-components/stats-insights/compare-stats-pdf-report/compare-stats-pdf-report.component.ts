import { CurrencyPipe, DatePipe, DecimalPipe, TitleCasePipe } from '@angular/common';
import { Component, EventEmitter, Input, OnChanges, OnInit, Output, SimpleChanges } from '@angular/core';
import { Message } from 'primeng/api';
import { CommonServiceService } from 'src/app/interface/service/common-service.service';
import { UserInteractionMessages } from 'src/assets/utilities/data/UserInteractionMessages.const';

import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { UserAuthService } from 'src/app/interface/auth-guard/user-auth/user-auth.service';
import { ServerCommunicationService } from 'src/app/interface/service/server-communication.service';

@Component({
	selector: 'dg-compare-stats-pdf-report',
	templateUrl: './compare-stats-pdf-report.component.html',
	styleUrls: ['./compare-stats-pdf-report.component.scss']
})

export class CompareStatsPdfReportComponent implements OnInit, OnChanges {

	@Output() messageOutPutForReportComponent = new EventEmitter<any>();
	@Input() dataForPdfReportComponent: object = {};
	@Input() finacialSelectedKey: string = 'turnover';
	@Input() selectedList: Object = {};
	@Input() payloadForCompare: any;
	@Input() IncludeEstimatedTurnover: boolean = false;

	reportGeneratedDate: Date = new Date();	
	constantMessages: any = UserInteractionMessages;
	messages: Message[] | undefined;
	listName: object = {
		firstName: '',
		secondName: ''
	};
	chipData: any;

	tableHeaderDataArray = [
		{ key: 'encorporationDate', label: 'Companies by Age', bgColor: '' },
		// { key: 'companiesOwnedObj', label: '', bgColor: '' },
		{ key: 'standardIndustryTags', label: 'Industry Tags', bgColor: '', addColumnData: 'standardIndustryTags' },
		{ key: 'otherMiscHandlerObj', label: 'Other Stats', bgColor: '' },
		{ key: 'compareIndustryListData', label: 'SIC Industry', bgColor: '' },
		{ key: 'employeesRangeArrayData', label: 'Number of Employees', bgColor: '' },
	]

	financialTableData = {
		turnover: [ 
			{ sectionHeader: 'Turnover ( £ )', dataKey: 'turnoverArray', payloadKey: 'turnover', countryAccess: [ 'uk' ]  },
			{ sectionHeader: 'Turnover Growth 1 Year', dataKey: 'turnoverGrowth1YearInfo', payloadKey: 'turnover', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Turnover Growth 3 Years', dataKey: 'turnoverGrowth3YearsInfo', payloadKey: 'turnover', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Turnover Growth 5 Years', dataKey: 'turnoverGrowth5YearsInfo', payloadKey: 'turnover', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Turnover (Include Estimated Turnover)', dataKey: 'turnoverPlusEstimatedTurnoverArray', payloadKey: 'turnover', countryAccess: [ 'uk', 'ie' ]  }
		],
		netWorth: [
			{ sectionHeader: 'Net Worth', dataKey: 'netWorthArray', payloadKey: 'netWorth', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Net Worth Growth 1 Year', dataKey: 'netWorthGrowth1YearInfo', payloadKey: 'net_worth', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Net Worth Growth 3 Years', dataKey: 'netWorthGrowth3YearsInfo', payloadKey: 'net_worth', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Net Worth Growth 5 Years', dataKey: 'netWorthGrowth5YearsInfo', payloadKey: 'net_worth', countryAccess: [ 'uk', 'ie' ] },
		],
		profitBeforeTax: [
			{ sectionHeader: 'Profit Before Tax', dataKey: 'profitBeforeTaxArray', payloadKey: 'profitBeforeTax' , countryAccess: [ 'uk', 'ie' ]  },
			// { sectionHeader: 'Profit Before Tax Growth 1 Year', dataKey: 'profitBeforeTaxGrowth1YearInfo', payloadKey: 'profit_before_tax', countryAccess: [ 'uk', 'ie' ]  },
			// { sectionHeader: 'Profit Before Tax Growth 3 Years', dataKey: 'profitBeforeTaxGrowth3YearsInfo', payloadKey: 'profit_before_tax', countryAccess: [ 'uk', 'ie' ]  },
			// { sectionHeader: 'Profit Before Tax Growth 5 Years', dataKey: 'profitBeforeTaxGrowth5YearsInfo', payloadKey: 'profit_before_tax', countryAccess: [ 'uk', 'ie' ]  },
		],
		totalAssets: [
			{ sectionHeader: 'Total Assets', dataKey: 'totalAssetsArray', payloadKey: 'totalAssets', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Total Assets Growth 1 Year', dataKey: 'totalAssetsGrowth1YearInfo', payloadKey: 'total_assets', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Total Assets Growth 3 Years', dataKey: 'totalAssetsGrowth3YearsInfo', payloadKey: 'total_assets', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Total Assets Growth 5 Years', dataKey: 'totalAssetsGrowth5YearsInfo', payloadKey: 'total_assets', countryAccess: [ 'uk', 'ie' ]  },
		],
		totalLiabilities: [
			{ sectionHeader: 'Total Liabilities', dataKey: 'totalLiabilitiesArray', payloadKey: 'totalLiabilities', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Total Liabilities Growth 1 Year', dataKey: 'totalLiabilitiesGrowth1YearInfo', payloadKey: 'total_liabilities', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Total Liabilities Growth 3 Years', dataKey: 'totalLiabilitiesGrowth3YearsInfo', payloadKey: 'total_liabilities', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Total Liabilities Growth 5 Years', dataKey: 'totalLiabilitiesGrowth5YearsInfo', payloadKey: 'total_liabilities', countryAccess: [ 'uk', 'ie' ]  },
		],
		tradeDebtors: [
			{ sectionHeader: 'Trade Debtors', dataKey: 'tradeDebtorsArray', payloadKey: 'tradeDebtors', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Trade Debtors Growth 1 Year', dataKey: 'tradeDebtorsGrowth1YearInfo', payloadKey: 'trade_debtors', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Trade Debtors Growth 3 Years', dataKey: 'tradeDebtorsGrowth3YearsInfo', payloadKey: 'trade_debtors', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Trade Debtors Growth 5 Years', dataKey: 'tradeDebtorsGrowth5YearsInfo', payloadKey: 'trade_debtors', countryAccess: [ 'uk', 'ie' ]  },
		],
		numberOfEmployees: [
			// { sectionHeader: 'Number of Employees', dataKey: 'noOfEmployeesArray', payloadKey: '', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Number of Employees Growth 1 Year', dataKey: 'numberOfEmployeesGrowth1YearInfo', payloadKey: 'number_of_employees', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Number of Employees Growth 3 Years', dataKey: 'numberOfEmployeesGrowth3YearsInfo', payloadKey: 'number_of_employees', countryAccess: [ 'uk', 'ie' ]  },
			{ sectionHeader: 'Number of Employees Growth 5 Years', dataKey: 'numberOfEmployeesGrowth5YearsInfo', payloadKey: 'number_of_employees', countryAccess: [ 'uk', 'ie' ]  },
		]
	}

	innerTableBorderColor = ['#d8d8d8', '#d8d8d8', '#d8d8d8', '#d8d8d8']

	constructor(
		public commonService: CommonServiceService,
		private toDatePipe: DatePipe,
		private decimalPipe: DecimalPipe,
		private toCurrencyPipe: CurrencyPipe,
		private titlecasePipe: TitleCasePipe,
		private userAuthService: UserAuthService,
		private globalServiceCommnunicate: ServerCommunicationService
	) { }

	ngOnInit() { }

	ngOnChanges(changes: SimpleChanges) {}
	
	async checkPdfReportLimit() {

		const firstSelection = this.selectedList['firstList'];
		const secondSelection = this.selectedList['secondList'];

		this.listName['firstName'] = firstSelection?.listName ?? firstSelection?.filterName;
		this.listName['secondName'] = secondSelection?.listName ?? secondSelection?.filterName;
		const base64Image = 'data:image/png;base64,';

		/**  Data Object For PDF */
		let docDefinition = {
			pageSize: 'A4',
			pageMargins: [10, 10, 10, 26],
			pageBorder: [true, true, true, true],
			background: function (currentPage, pageSize) {
				return {
					canvas: [
						{
							type: 'rect',
							x: 0, y: 0,
							w: pageSize.width,
							h: pageSize.height,
							color: '#fff',
						}
					]
				};
			},
			content: [
				{
					layout: {
						defaultBorder: false
					},
					table: {
						headerRows: 0,
						widths: ['*'],
						heights: ['*'],
						body: [
							[
								{
									image: await this.commonService.convertImageToDataUrl( 'compareStatsCoverImage.jpg' ),
									width: 560,
									height: 795,
									alignment: 'center'
								}
							]
						]
					}
				},
				{
					layout: {
						defaultBorder: false
					},
					table: {
						headerRows: 0,
						widths: ['*'],
						body: [
							[
								{
									image: await this.commonService.convertImageToDataUrl( 'dgLogoPdfDark.png' ),
									width: 180,
									margin: [0, 15, 0, 10],
									alignment: 'center'
								}
							]
						]
					}
				},
				{
					text: `Report Generated Date: ${this.toDatePipe.transform(this.reportGeneratedDate, 'dd-MM-yyyy, HH:mm')}`,
					fontSize: 7,
					alignment: 'right',
					margin: [0, -20, 0, 10],
				},
				{
					layout: {
						defaultBorder: false
					},
					table: {
						headerRows: 0,
						widths: [ '*', '*', '*', '*', '*' ],
						body: [this.tableTypeDataBody()]
					}
				}
			],
			styles: {
				headingStyle: {
					fontSize: 10,
					color: '#fff'
				},
				headingStyle2: {
					fontSize: 10,
					color: '#1f4599'
				},
				subHeadingStyle: {
					fontSize: 8,
					bold: true,
					color: '#1f4599'
				},
				subHeadStyle: {
					fontSize: 8,
					lineHeight: 1.3,
					color: '#505050'
				},
				innerTableHeaderStyle: {
					fillColor: '#D3D3D3',
					fontSize: 6,
					color: '#333',
					bold: true
				},
				paraTextStyle: {
					fontSize: 6,
					lineHeight: 1.3,
					color: '#505050'
				},
				disclaimerTextStyle: {
					fontSize: 7,
					lineHeight: 1.3,
					color: '#0000'
				},
				msmeStyle: {
						margin: [5, 5, 5, 5],
						alignment: 'center',
						color: '#fff'
				}
			},
			pageBreakBefore: (currentNode, followingNodesOnPage, nodesOnNextPage, previousNodesOnPage) => {
				let nodeHasTable = currentNode.table,
					tableRowsPositionOnPageEnd = currentNode.startPosition.top > currentNode.startPosition.pageInnerHeight - 40;

				return nodeHasTable && tableRowsPositionOnPageEnd;
			},
			footer: function(currentPage, pageCount) {
				if (currentPage > 1  && currentPage < pageCount) { 
					return [
						{
							width: 600,
							// height: 32,
							image: base64Image,
							alignment: 'center',
						},
					];
				} else {
					return []
				}
			},
		};

		/** open/download:- data object */

		let fileName, pdfFile;
		fileName = "DataGardener_Statistical_Analysis_" + this.formatDate(this.reportGeneratedDate) +'_' + new Date().getTime() + '_Report.pdf';

        // return pdfMake.createPdf(docDefinition).open();

		pdfMake.createPdf(docDefinition).download(fileName);
		const pdfDocGenerator = pdfMake.createPdf(docDefinition);

		pdfDocGenerator.getBase64((data) => {

			pdfFile = data;

			let pdfReportObj = {
				userId: this.userAuthService?.getUserInfo('dbID'),
				fileName: fileName,
				reportType: 'Business Statistics Report',
				pdfFile: pdfFile
			}

			this.globalServiceCommnunicate.globalServerRequestCall('post', 'DG_API', 'uploadPdfToS3', pdfReportObj).subscribe( res => {
				this.messages = [];
				this.messages.push({ severity: 'success', summary: "Report Downloaded Successfully" });
				this.messageOutPutForReportComponent.emit(this.messages);
				setTimeout(() => {
					this.messages = [];
					this.messageOutPutForReportComponent.emit(this.messages);
				}, 2000);
			});

		});

	}

	formatDate(date) {
		var d = new Date(date),
			month = '' + (d.getMonth() + 1),
			day = '' + d.getDate(),
			year = d.getFullYear();

		if (month.length < 2)
			month = '0' + month;
		if (day.length < 2)
			day = '0' + day;

		return [day, month, year].join('-');
	}

	tableTypeDataBody() {

		let tableBodyArray: Array<any> = [];

		let tableBodyObj = {
			colSpan: 5,
			layout: {
				defaultBorder: false,
				paddingLeft: function (i, node) { return 5; },
				paddingRight: function (i, node) { return 5; }
			},
			fillColor: '#fff',
			table: {
				headerRows: 0,
				widths: ['*'],
				body: this.tableHandler()
			}
		}

		tableBodyArray.push(tableBodyObj);
		return tableBodyArray;

	}

	tableHandler() {

		let tempArray = [];
		let blankSpaceRow: Array<any> = [{ text: '', margin: [0, 0, 0, 0] }];

		

		/***Header financial Card */
		tempArray.push( [ { text: 'Summary', style: 'headingStyle', fillColor: '#008f9a', bold: true } ] )
		tempArray.push( this.headerCard() );
		tempArray.push( blankSpaceRow );
		/****End Here */

		/*** MSME Table */
		tempArray.push( [ { text: 'MSME Tags', style: 'headingStyle', fillColor: '#008f9a', bold: true } ] )
		tempArray.push( this.msmeTable() );
		tempArray.push( blankSpaceRow );
		/****End Here */

		/*** RISK Table */
		tempArray.push( [ { text: 'Risk Analysis', style: 'headingStyle', fillColor: '#008f9a', bold: true } ] )
		tempArray.push( this.riskAnalysisTable() );
		tempArray.push( blankSpaceRow );
		/****End Here */

		/*** Same field comparison table */
		for ( let item of this.tableHeaderDataArray ) {

			let header = [ { text: item?.label, style: 'headingStyle', fillColor: '#008f9a', bold: true } ];
			const{ firstItem, secondItem } = this.dataForPdfReportComponent[ item?.key ];
			let columnWidth = [ '*', 80, 80, 80, 80 ];

			if ( item?.key == 'standardIndustryTags' ) {
				columnWidth = [ '*', 80, 80, '*', 80, 80 ]
			}

			let dataRow = [
				{
					layout: {
						paddingTop: function (i, node) { return 2; },
						paddingBottom: function (i, node) { return 2; },
					
					},
					table: {
						headerRows: 0,
						widths: columnWidth,
						body: this.displayItemValue( firstItem, secondItem, item?.addColumnData, item )
					},
				}
			]

			tempArray.push( header );
			tempArray.push( dataRow );
			tempArray.push( blankSpaceRow );
		}

		/*** financial Table Data */
		for( let val of this.financialTableData[ this.finacialSelectedKey ] ) {
			let finHeader = [ { text: val?.sectionHeader, style: 'headingStyle', fillColor: '#008f9a', bold: true } ];

			const { firstItem, secondItem } = this.dataForPdfReportComponent['keyForFinancial'];
			let finRow = [
				{
					layout: {
						paddingTop: function (i, node) { return 2; },
						paddingBottom: function (i, node) { return 2; },
					
					},
					table: {
						headerRows: 0,
						widths: [ '*', 80, 80, 80, 80 ],
						body: this.displayItemValue( firstItem[ val?.dataKey ], secondItem[ val?.dataKey ], undefined, val )
					},
				}
			]

			tempArray.push( finHeader );
			tempArray.push( finRow );
			tempArray.push( blankSpaceRow );
		}

		return tempArray;
	}

	displayItemValue( firstItem, secondItem, key?, chipGroup? ) {

		let tempArray = [],mainArray = [], secondArray = [];
		let itemLength = firstItem.length >= secondItem.length;

		if ( itemLength ) {
			mainArray = firstItem;
			secondArray = secondItem;
		} else {
			mainArray = secondItem;
			secondArray = firstItem;
		}

		for ( let i = 0; i < mainArray.length; i++ ) {

			let tempObj = { 
				label: mainArray[i]['label'], 
				first_count: this.toCurrencyPipe.transform(mainArray[i]['count'] ?? 0, '', '', '1.0-2'), 
				first_count_percentage: this.toCurrencyPipe.transform(mainArray[i]['count_percentage'] ?? 0, '', '', '1.2-2') + " %",
				first_payload: mainArray?.[i]?.['value'],
				second_payload: secondArray?.[i]?.['value'],
				second_count: this.toCurrencyPipe.transform(secondArray?.[i]?.['count'] ?? 0, '', '', '1.0-2'), 
				second_count_percentage: this.toCurrencyPipe.transform(secondArray?.[i]?.['count_percentage'] ?? 0, '', '', '1.2-2') + " %" 
			}
			if ( key ) {
				tempObj[ 'label2' ] = secondArray?.[i]?.['label'] ?? '-';
			}
			tempArray.push( tempObj )

		}

		let tableData = [];
		let count = 0, fillColorVar = ['#F0F0F0', '#fff'];
		
		let mainListNameHeader = [
			{ text: '', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor },
			{ text: this.listName['firstName'], style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor, alignment: 'center', colSpan:2 },
			{ text: '', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor},
			{ text: this.listName['secondName'], style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor, alignment: 'center', colSpan:2 },
			{ text: '', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor},
				
		];

		if ( key ) {
			mainListNameHeader.splice( 3, 0, { text: '', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor} );
		}

		let fieldNameHeader = [
			{ text: '', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor },
			{ text: 'Count', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor, alignment: 'right' },
			{ text: 'Percentage', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor, alignment: 'right' },
			{ text: 'Count', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor, alignment: 'right' },
			{ text: 'Percentage', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor, alignment: 'right' }
		
		];

		if ( key ) {
			fieldNameHeader.splice( 3, 0, { text: '', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor});
		}
		
		tableData.push(mainListNameHeader);
		tableData.push(fieldNameHeader);

		for ( let item of tempArray ) {

			let itemData = { text: this.titlecasePipe.transform( item?.label ), style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor };

			let firstitem: any

			if( item?.first_count == 0 ) {
				firstitem = { text: item?.first_count, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor, alignment: 'right'};
			} else {
				firstitem = { text: item?.first_count, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor, alignment: 'right', color: '#4593bd', link: this.goToSearchPage( item, 'firstItem', this.listName['firstName'], chipGroup, item.first_payload )}
			}

			let secondArray: any;

			if( item?.second_count == 0 ) {
				secondArray = { text: item?.second_count, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor, alignment: 'right'};
			} else {
				secondArray = { text: item?.second_count, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor, alignment: 'right', color: '#4593bd', link: this.goToSearchPage( item, 'secondItem', this.listName['secondName'], chipGroup, item.second_payload) }
			}
			
			let bodyContent = [
				itemData,
				firstitem,
				{ text: item?.first_count_percentage, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor, alignment: 'right' },
				secondArray,
				{ text: item?.second_count_percentage, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor, alignment: 'right' },
			];

			if ( key ) {
				bodyContent.splice( 3, 0, { text: this.titlecasePipe.transform( item?.label2 ) ?? '-', style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor } );
			}

			tableData.push(bodyContent);
			count++;
		}

		return tableData;

	}
	goToSearchPage(val, item, listName, chipGroup?, financialPayload?){
		let chipData =  this.payloadForCompare[ item ].type !== 'list' ? this.payloadForCompare[ item ].id : [{chip_group: "Saved Lists", chip_values: [listName]}];
		let urlStr: string;

		if( chipGroup){

			if( chipGroup?.label == 'Companies by Age') {
				chipData = chipData.filter( val => val.chip_group != 'Company Age Filter' );
				if( val.label == '0 to 2'){
					val.label = [0, 2]
				} else if(val.label == '2 to 5') {
					val.label = [2, 5]
				}else if(val.label == '5 to 10') {
					val.label = [5, 10]
				}else if(val.label == '10+') {
					val.label = [10, 0]
				}else if(val.label == 'Not Specified') {
					val.label = ['not specified']
				}
				chipData.push(
					{
						chip_group: 'Company Age Filter',
						chip_values: [ val.label ],
						label: 'Company Age'
					}
				);
			} else if ( chipGroup?.label == 'Industry Tags') {
				chipData = chipData.filter( val => val.chip_group != 'Industry Tags' );
			
				chipData.push(
					{
						chip_group: 'Industry Tags',
						chip_values: val.label == 'Not Specified' ? ['not specified'] : [ val.label ],
						filter_exclude: false
					}
				);
			} else if ( chipGroup?.label == 'Other Stats') {
				chipData = chipData.filter( val => val.chip_group != 'Preferences' );
				let preferenceObj = {},
				chipValues = ''
				
				if (val.label == "All CCJ's") {
					chipValues = "Company must have CCJ";
					preferenceObj = { "hasCCJInfo": "true" };
				}
				if (val.label == 'Companies With Patent') { 
					chipValues = "company must have companies with patent";
					preferenceObj = { "hasPatentData": "true" };
				}
				if (val.label == 'Companies with Grants') { 
					chipValues = "company must have companies with grants";
					preferenceObj = { "hasInnovateData": "true" };
				}
				if (val.label == 'Write-offs') { 
					chipValues = "Company must have write-offs";
					preferenceObj = { "isDebtor": "true" };
				}
				if (val.label == 'Companies Land Ownerships') { 
					chipValues = "Company must have corporate land";
					preferenceObj = { "hasLandCorporate": "true" };
				}
				if (val.label == 'Companies with Shareholdings') { 
					chipValues = "Company must have shareholdings";
					preferenceObj = { "hasShareholdings": "true" };
				}
				if (val.label == 'Environment Agency Registered') { 
					chipValues = "Company must have ecs data";
					preferenceObj = { "hasEcsData": "true" };
				}
				if (val.label == 'Company With Charges') { 
					chipValues = "Company must have charges details";
					preferenceObj = { "hasCharges": "true" };
				}

				if ( val.label == "New CCJ's" ) {
					chipValues = "new judgment";
					chipData.push(
						{
							chip_group: 'CCJ Status',
							chip_values: [ chipValues ]
						}
					);
				}else {
					chipData.push(
						{
							chip_group: 'Preferences',
							chip_values: [ chipValues ],
							preferenceOperator: [preferenceObj]
						}
					);
				}
			} else if ( chipGroup?.label == 'SIC Industry') {
				chipData = chipData.filter( val => val.chip_group != 'SIC Codes' );
				// let sicCodesValue = val.label.split('- ');
				let sicCodesValue = val.label.substring(val.label.indexOf('- ') + 1).trim();
				chipData.push(
					{
						chip_group: 'SIC Codes',
						chip_industry_sic_codes: sicCodesValue == 'Not Specified' ? ['not specified'] : [ sicCodesValue ],
						chip_values: val.label == 'Not Specified' ? ['not specified'] : [ val.label ],
						filter_exclude: false,
						label: "SIC Codes"
					}
				);
			} else if ( chipGroup?.label == 'Number of Employees') {
				chipData = chipData.filter( val => val.chip_group != 'Number of Employees' );
             	let chipVal
				if( val.label == 'Not Specified' ) {
					chipVal = 'not specified'
				} else if ( val.label  == '1000+' ) {
					chipVal = ['1001', '0']
				} else {
					chipVal = val.label.split( ' to ')
				}
				chipData.push(
					{
						chip_group: 'Number of Employees',
						chip_values: chipVal == 'not specified' ? [['not specified']] : [[ +chipVal[0], +chipVal[1] ]],
					}
				);
			} else if (  ['Turnover ( £ )', 'Turnover (Include Estimated Turnover)'].includes( chipGroup?.sectionHeader) ){
				chipData = chipData.filter( val => val.chip_group != 'Key Financials' );
				if( chipGroup?.sectionHeader == 'Turnover (Include Estimated Turnover)'){
					chipData.push(
						{
							chip_group: 'Key Financials',
							
							chip_values: [
								{ 	financialBoolean: true,
									greater_than: financialPayload.greaterThan ? financialPayload.greaterThan : undefined, 
									key: "turnover", 
									less_than: financialPayload.lessThan ? financialPayload.lessThan : undefined, 
									selected_year: "true"
								},
								
								{ 	financialBoolean: true,
									greater_than: financialPayload.greaterThan ? financialPayload.greaterThan : undefined, 
									key: "estimated_turnover", 
									less_than: financialPayload.lessThan ? financialPayload.lessThan : undefined, 
									selected_year: "true"
								}
							]
						}
					);
				} else {
					chipData.push(
						{
							chip_group: 'Key Financials',
							
							chip_values: [
								{ 	financialBoolean: true,
									greater_than: financialPayload.greaterThan ? financialPayload.greaterThan : undefined, 
									key: "turnover", 
									less_than: financialPayload.lessThan ? financialPayload.lessThan : undefined, 
									selected_year: "true"
								}
							]
						}
					);
				}
			} else {
				chipData = chipData.filter( val => val.chip_group != chipGroup?.sectionHeader );

				let chipGrp
				if( chipGroup?.sectionHeader ){
					if( chipGroup?.sectionHeader == 'Turnover Growth 3 Years' ){
						chipGrp = '3 Years Growth';
					} else if ( chipGroup?.sectionHeader == 'Turnover Growth 1 Year'){
						chipGrp = '1 Year Growth';
					} else if ( chipGroup?.sectionHeader == 'Turnover Growth 5 Years'){
						chipGrp = '5 Years Growth';
					}
				}

				chipData.push(
					{
						chip_group: chipGrp,
						
						chip_values: [
							{
								greater_than: financialPayload.greaterThan ? financialPayload.greaterThan : undefined, 
								key: "turnover", 
								less_than: financialPayload.lessThan ? financialPayload.lessThan : undefined, 
								suffix: "%"
							}
						]
					}
				);
			}

		}
		let listId = this.payloadForCompare[ item ].type == 'list' ? this.payloadForCompare[ item ].id : ''

		if( listId ) {
			
			urlStr = `${window.location.origin}/company-search?chipData=${ encodeURIComponent( JSON.stringify(chipData) ) }&listPageName=${ 'companyListPage' }&cListId=${ listId }`;
		} else {
			urlStr = `${window.location.origin}/company-search?chipData=${ encodeURIComponent( JSON.stringify(chipData) ) }`;
		}

		return urlStr

	}

	headerCard() {
		let tableBody = [];

		

		let tableBodyObj = {
			layout: {
				defaultBorder: true,
				paddingLeft: function (i, node) { return 4; },
				paddingRight: function (i, node) { return 4; }
			},
			fillColor: '#fff',
			table: {
				headerRows: 1,
				widths: ['*', 80, 80 ],
				body: this.mainHeaderTable()
			}
		}

		tableBody.push( tableBodyObj );
		return tableBody
	}

	mainHeaderTable() {

		let tableData = [], count = 0, fillColorVar = ['#F0F0F0', '#fff'];

		let mainHeaderCard = {
			greenCard: '#1ab394',
			voiletCard: '#5f52df',
			blueCard: '#0086d6'
		}
	
		let mainListNameHeader = [
			{ text: '', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor },
			{ text: this.listName['firstName'], style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor, alignment: 'center' },
			{ text: this.listName['secondName'], style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor, alignment: 'center' },
				
		];

		// let fieldNameHeader = [
		// 	{ text: '', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor },
		// 	{ text: 'count', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor },
		// 	{ text: 'Count', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor },
		
		// ];
		
		tableData.push( mainListNameHeader );
		// tableData.push( fieldNameHeader );

		for ( let val of this.dataForPdfReportComponent['headerData'] ) {

			let rowItem = [
				{ text: val.header, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor  },
				{ text: val.firstItem_count, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor , alignment: 'right' },
				{ text: val.secondItem_count, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor , alignment: 'right' },
			]
			count++;
			tableData.push( rowItem );
		}
		return tableData;
	}

	msmeTable() {

		let tableBodyContent = [], msmeBody = [];
		let count = 0, fillColorVar = ['#F0F0F0', '#fff'];


		let mainListNameHeader = [
			{ text: '', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor },
			{ text: this.listName['firstName'], style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor, alignment: 'center' },
			{ text: this.listName['secondName'], style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor, alignment: 'center' },
				
		];

		let fieldNameHeader = [
			{ text: '', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor },
			{ text: 'Count', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor, alignment: 'right' },
			{ text: 'Count', style: 'innerTableHeaderStyle', borderColor: this.innerTableBorderColor, alignment: 'right' },
		
		];

		msmeBody.push( mainListNameHeader );
		msmeBody.push( fieldNameHeader );

		for ( let val of this.dataForPdfReportComponent['msmeDataArrayForPDF'] ) {
			let itemData = { text: val.label, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor };

			let firstitem: any

			if( val.firstItem_count == 0 ) {
				firstitem = { text: val.firstItem_count, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor, alignment: 'right' };
			} else {
				firstitem = { text: val.firstItem_count, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor, alignment: 'right', color: '#4593bd', link: this.goToMsmeData(val,'firstItem', this.listName['firstName']) }
			}

			let secondArray: any;

			if( val.secondItem_count == 0 ) {
				secondArray = { text: val.secondItem_count, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor, alignment: 'right' };
			} else {
				secondArray = { text: val.secondItem_count, style: 'paraTextStyle', fillColor: fillColorVar[count % 2], borderColor: this.innerTableBorderColor, alignment: 'right', color: '#4593bd', link: this.goToMsmeData(val, 'secondItem', this.listName['secondName']) }
			}
			let rowItem = [
				itemData,
				firstitem,
				secondArray,
			]

			count++;
			msmeBody.push( rowItem );
		}

		let tableBodyobj = {
			layout: {
				defaultBorder: true,
				paddingLeft: function (i, node) { return 4; },
				paddingRight: function (i, node) { return 4; }
			},
			fillColor: '#fff',
			table: {
				widths: [ '*', 80, 80 ],
				body: msmeBody
			}
		}

		tableBodyContent.push( tableBodyobj );
		return tableBodyContent;
	}

	goToMsmeData(val,item, listName) {

		let chipData =  this.payloadForCompare[ item ].type !== 'list' ? this.payloadForCompare[ item ].id : [{chip_group: "Saved Lists", chip_values: [listName]}]

		let navigateUrlParamObject = JSON.parse( JSON.stringify( chipData ) );
		let urlStr: string;

		navigateUrlParamObject = navigateUrlParamObject.filter(val => val.chip_group !== 'MSME Classification');
		navigateUrlParamObject.push(
			{
				chip_group: 'MSME Classification',
				chip_values: [ val.label ]
			}
		);

		let listId = this.payloadForCompare[ item ].type == 'list' ? this.payloadForCompare[ item ].id : ''

		if( listId   ) {
			urlStr = `${window.location.origin}/company-search?chipData=${ encodeURIComponent( JSON.stringify(navigateUrlParamObject) ) }&listPageName=${ 'companyListPage' }&cListId=${ listId }`;
		} else {
			urlStr = `${window.location.origin}/company-search?chipData=${ encodeURIComponent( JSON.stringify(navigateUrlParamObject) ) }`;
		}

		return urlStr;
	}

	riskAnalysisTable() {	

		let tableBodyContent = [], riskBody = [];


		let mainListNameHeader = [
			{ text: '', style: 'innerTableHeaderStyle', borderColor: ['#fff', '#fff', '#fff', '#fff'] },
			{ text: this.listName['firstName'], style: 'innerTableHeaderStyle', borderColor: ['#fff', '#fff', '#fff', '#fff'], colSpan: 2, alignment: 'center' },
			{ text: '', style: 'innerTableHeaderStyle', borderColor: ['#fff', '#fff', '#fff', '#fff']},
			{ text: this.listName['secondName'], style: 'innerTableHeaderStyle', borderColor: ['#fff', '#fff', '#fff', '#fff'], colSpan: 2, alignment: 'center'  },
			{ text: '', style: 'innerTableHeaderStyle', borderColor: ['#fff', '#fff', '#fff', '#fff']}
				
		];

		let fieldNameHeader = [
			{ text: '', style: 'innerTableHeaderStyle', borderColor: ['#fff', '#fff', '#fff', '#fff'] },
			{ text: 'Count', style: 'innerTableHeaderStyle', borderColor: ['#fff', '#fff', '#fff', '#fff'], alignment: 'right' },
			{ text: 'Percentage', style: 'innerTableHeaderStyle', borderColor: ['#fff', '#fff', '#fff', '#fff'], alignment: 'right' },
			{ text: 'Count', style: 'innerTableHeaderStyle', borderColor: ['#fff', '#fff', '#fff', '#fff'], alignment: 'right' },
			{ text: 'Percentage', style: 'innerTableHeaderStyle', borderColor: ['#fff', '#fff', '#fff', '#fff'], alignment: 'right' },
		
		];

		riskBody.push( mainListNameHeader );
		riskBody.push( fieldNameHeader );

		for ( let val of this.dataForPdfReportComponent['riskChartLabel'] ) {
			let labelDetail = { text: val.label, fillColor: val.bckgColor, color: '#fff', fontSize: 9, borderColor: ['#fff', '#fff', '#fff', '#fff'] };
			let firstItem_count_percentage = { text: this.toCurrencyPipe.transform(val?.['firstItem_count_percentage'] ?? 0, '', '', '1.2-2') + " %", fillColor: val.bckgColor, color: '#fff', fontSize: 9, borderColor: ['#fff', '#fff', '#fff', '#fff'], alignment: 'right' };
			let secondItem_count_percentage  = { text: this.toCurrencyPipe.transform(val?.['secondItem_count_percentage'] ?? 0, '', '', '1.2-2') + " %", fillColor: val.bckgColor, color: '#fff', fontSize: 9, borderColor: ['#fff', '#fff', '#fff', '#fff'], alignment: 'right' }
			let firstitem: any

			if( val?.firstItem_count == 0 ) {
				firstitem = { text: val?.firstItem_count ?? 0, fillColor: val.bckgColor, fontSize: 9, borderColor: ['#fff', '#fff', '#fff', '#fff'], alignment: 'right' };
			} else {
				firstitem = { text: val?.firstItem_count ?? 0, fillColor: val.bckgColor, fontSize: 9, borderColor: ['#fff', '#fff', '#fff', '#fff'], alignment: 'right', link: this.goToRiskAnalysisData(val.field,'firstItem', this.listName['firstName']) }
			}

			let secondArray: any;

			if( val?.secondItem_count == 0 ) {
				secondArray = { text: val?.secondItem_count ?? 0, fillColor: val.bckgColor, fontSize: 9, borderColor: ['#fff', '#fff', '#fff', '#fff'], alignment: 'right' };
			} else {
				secondArray = { text: val?.secondItem_count ?? 0, fillColor: val.bckgColor, fontSize: 9, borderColor: ['#fff', '#fff', '#fff', '#fff'], alignment: 'right', link: this.goToRiskAnalysisData(val.field,'secondItem', this.listName['secondName']) }
			} 
			
			let rowItem = [
				labelDetail,
				firstitem,
				firstItem_count_percentage,
				secondArray,
				secondItem_count_percentage,
			]

			riskBody.push( rowItem );
		}

		let tableBodyobj = {
			layout: {
				defaultBorder: true,
				paddingLeft: function (i, node) { return 4; },
				paddingRight: function (i, node) { return 4; }
			},
			fillColor: '#fff',
			table: {
				widths: [ '*', 80, 80, 80, 80 ],
				body: riskBody
			}
		}

		tableBodyContent.push( tableBodyobj );
		return tableBodyContent;

	}
	goToRiskAnalysisData( bandName,item, listName ) {
		
		let chipData =  this.payloadForCompare[ item ].type !== 'list' ? this.payloadForCompare[ item ].id : [{chip_group: "Saved Lists", chip_values: [listName]}];

		let navigateUrlParamObject = JSON.parse( JSON.stringify( chipData ) );
		let urlStr: string;

		navigateUrlParamObject = navigateUrlParamObject.filter(val => val.chip_group !== 'Bands');

		let bandName_chipVal = this.formatTextNameForTables( bandName )

		navigateUrlParamObject.push(
			{
				chip_group: 'Bands',
				chip_values: [ bandName_chipVal ],
				label: 'Credit Risk Bands'
			}
		);

		let listId = this.payloadForCompare[ item ].type == 'list' ? this.payloadForCompare[ item ].id : ''

		if( listId ) {
			urlStr = `${window.location.origin}/company-search?chipData=${ encodeURIComponent( JSON.stringify(navigateUrlParamObject) ) }&listPageName=${ 'companyListPage' }&cListId=${ listId }`;
		} else {
			urlStr = `${window.location.origin}/company-search?chipData=${ encodeURIComponent( JSON.stringify(navigateUrlParamObject) ) }`;
		}

		return urlStr;
	}

	formatTextNameForTables(finalName) {
		if (finalName == 'moderateRisk') {
			finalName = 'moderate risk'
		} else if (finalName == 'lowRisk') {
			finalName = 'low risk'
		} else if (finalName == 'veryLowRisk') {
			finalName = 'very low risk'
		} else if (finalName == 'notScored') {
			finalName = 'Not Scored /Very High Risk'
		} else if (finalName == 'highRisk') {
			finalName = 'high risk'
		} else if (finalName == 'Ccj') {
			finalName = 'County Court Judgments'
		} else if (finalName == 'Patentdata') {
			finalName = 'Companies With Patent'
		} else if (finalName == 'Innovatedata') {
			finalName = 'Companies with Grants'
		} else if (finalName == 'Writeoff') {
			finalName = 'Write-offs'
		} else if (finalName == 'Landcoporatedetails') {
			finalName = 'Companies Land Ownerships'
		} else if (finalName == 'Shareholding') {
			finalName = 'Companies with Shareholdings'
		} else if (finalName == 'Ecsdata') {
			finalName = 'Environment Agency Registered'
		} else if (finalName == 'Negativeturnover') {
			finalName = 'Zero Value'
		} else if (finalName == 'Lessthan1m') {
			finalName = 'Less Than 1M'
		} else if (finalName == '1Mto5m') {
			finalName = '1M to 5M'
		} else if (finalName == '5Mto10m') {
			finalName = '5M to 10M'
		} else if (finalName == '10Mto100m') {
			finalName = '10M to 100M'
		} else if (finalName == '100Mto500m') {
			finalName = '100M to 500M'
		} else if (finalName == '500Mto1b') {
			finalName = '500M to 1B'
		} else if (finalName == 'Greaterthan1b') {
			finalName = 'Greater Than 1B'
		} else if (finalName == '0-2') {
			finalName = '0 to 2'
		} else if (finalName == '3-5') {
			finalName = '3 to 5'
		} else if (finalName == '6-10') {
			finalName = '6 to 10'
		} else if (finalName == '11-25') {
			finalName = '11 to 25'
		} else if (finalName == '26-50') {
			finalName = '26 to 50'
		} else if (finalName == '51-100') {
			finalName = '51 to 100'
		} else if (finalName == '101-250') {
			finalName = '101 to 250'
		} else if (finalName == '251-500') {
			finalName = '251 to 500'
		} else if (finalName == '501-1000') {
			finalName = '501 to 1000'
		} else if (finalName == '1To1m') {
			finalName = '1 to 1M'
		} else if ( finalName )	{
			finalName = finalName
		}
		return finalName;
	}
}
