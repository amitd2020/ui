...
@Component({
	selector: 'dg-contract-finder-saved-filters',
	templateUrl: './contract-finder-saved-filters.component.html',
	styleUrls: ['./contract-finder-saved-filters.component.scss']
})
export class ContractFinderSavedFiltersComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.tableCols = [
			{ field: 'filterName', header: 'Filter Name', minWidth: "350px", maxWidth: '350px', textAlign: "left" },
			{ field: 'searchType', header: 'Page', minWidth: "200px", maxWidth: '200px', textAlign: "left" },
			{ field: 'chipData', header: 'Filter Criteria', minWidth: "450px", maxWidth: 'none', textAlign: "left" },
	...
	getSavedFilterData(pageSize?, pageNumber?) {
		...
		this.serverCommunicate.globalServerRequestCall('get', 'DG_LIST', 'getGovtProcSaveFilteredLists', params).subscribe(res => {
			this.tableData = res.body['results'];
			this.totalNumOfRecords = res.body['totalResults'];
	...
	getUpdateTableDataList(event) {
		...
		this.getSavedFilterData(event.event.rows, event.event.page + 1);
	...
	editFunction(filterName, userID, _id) {
		...
		this.serverCommunicate.globalServerRequestCall('post', 'DG_LIST', 'updateFilterName', requestBody).subscribe(res => {
	...
	deleteList() {
		...
		this.serverCommunicate.globalServerRequestCall('post', 'DG_LIST', 'deleteUserFilterCriteria', requestBody).subscribe(res => {
