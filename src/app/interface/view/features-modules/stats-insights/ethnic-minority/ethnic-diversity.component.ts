...
@Component({
	selector: 'dg-ethnic-diversity',
	templateUrl: './ethnic-diversity.component.html',
	styleUrls: ['../stats-insights-dashboard.component.scss', './ethnic-diversity.component.scss']
})
export class EthnicDiversityComponent implements OnInit {

	@ViewChild( 'LazyLeafletMapContainer', { read: ViewContainerRef } ) LazyLeafletMapContainer: ViewContainerRef;

	@ViewChild('ethnicDiversityTable', { static: false }) public ethnicDiversityTable: Table;

	ChartDataLabelsPlugins = [ ChartDataLabels ];

	title: string = '';
	description: string = '';
	ethnicDiversityDataValue : any;
	...
	employeesRangeArrayData: Array<{ field: string, label: string, count: number, count_percentage: number, rangeFrom: string, rangeTo: string }> = [
		...
		{ field: '1000+', label: '1000+',count: 0, count_percentage: 0, rangeFrom: '1000', rangeTo: '' }
	];
	...
	ethnicDiversityDataObject: any = {
		"filterData": [
			...
			},
			{
				...
				"chip_values": [ "Company must be ethnic minorities" ],
	...
	ngOnInit() {
		...
		this.getdataForEthenicMinority();
	}

	initBreadcrumbAndSeoMetaTags() {
		
		// this.breadcrumbService.setItems([
		// 	{ label: 'Ethnic Diversity', routerLink: ['/stats-insights/ethnic-diversity'] }
		// ]);
		this.title = "DataGardener Ethnic Diversity - Automate your marketing workflows";
	...
	getdataForEthenicMinority() {

		this.globalServerCommunication.globalServerRequestCall('post', 'DG_CHART_API', 'getEthnicStatsData', this.ethnicDiversityDataObject ).subscribe(res => {
					
			if ( res.body.status == 200 ) {
				
				this.ethnicDiversityDataValue = res.body.result;
				let nonEthnicCounts = this.ethnicDiversityDataValue?.totalEthnicCounts?.nonEthnicCounts?.companies;
				let ethnicCount = this.ethnicDiversityDataValue?.totalEthnicCounts?.ethnicCounts?.companies;
				...
				this.ethenicGenderCount = this.ethnicDiversityDataValue.foundedCompaniesCounts;

				// Turonver Counts Start
				if ( this.ethnicDiversityDataValue.financialTurnoversArray ) {

					for ( let turonver of this.financialTurnoversArrayData ) {

						for ( let statsFinTurnover of this.ethnicDiversityDataValue.financialTurnoversArray ) {
				...
				if ( this.ethnicDiversityDataValue.financialTurnoversPlusEstimatedTurnoversArray ) {

					for ( let finTurnover of this.estimatedTurnoversArrayData ) {
						
						for ( let estimatedStatsFinTurnover of this.ethnicDiversityDataValue.financialTurnoversPlusEstimatedTurnoversArray ) {
				...
				if ( this.ethnicDiversityDataValue.financialTotalAssetsArray ) {

					for ( let totalAssets of this.financialTotalAssetsArrayData ) {
	
						for ( let statsFinTurnover of this.ethnicDiversityDataValue.financialTotalAssetsArray ) {
				...
				if ( this.ethnicDiversityDataValue.financialNetworthArray ) {

					for ( let netWorth of this.financialNetworthArrayData ) {
	
						for ( let statsFinTurnover of this.ethnicDiversityDataValue.financialNetworthArray ) {
				...
				for (let i = 0; i < this.industryListData.length; i++) {
					...
					for (let j = 0; j < this.ethnicDiversityDataValue.industry.length; j++) {
						
						if ( this.industryListData[i]['value'] == this.ethnicDiversityDataValue.industry[j]['industry'] ) {

							this.industryListData[i]['count'] = this.ethnicDiversityDataValue.industry[j]['count'];
							this.industryListData[i]['count_percentage'] = this.ethnicDiversityDataValue.industry[j]['count_percentage'];
						}
					}
				...
				if ( this.ethnicDiversityDataValue.companyIncorporationInfo ) {
					
					for ( let employeeKey of this.encorporationDate ) {
	
						for ( let employeeKeyRange of this.ethnicDiversityDataValue.companyIncorporationInfo ) {
				...
				if ( this.ethnicDiversityDataValue.noOfEmployeesArray ) {

					for ( let employeeKey of this.employeesRangeArrayData ) {
						
						for ( let employeeKeyRange of this.ethnicDiversityDataValue.noOfEmployeesArray ) {
				...
				if ( this.ethnicDiversityDataValue.otherMisc ) {

					for ( let otherMiscObj of this.otherMiscArrayData ) {
						otherMiscObj.count = this.ethnicDiversityDataValue.otherMisc[ otherMiscObj.field ];
						otherMiscObj.count_percentage = this.ethnicDiversityDataValue.otherMisc[ `${ otherMiscObj.field }_percentage` ];
					}
				...
				if ( this.ethnicDiversityDataValue.riskChart ) {

					this.initDoughnutChartContainer( this.ethnicDiversityDataValue.riskChart, "riskChart" );
	
				}
				// Risk Analysis Chart Ends

				// Region Map Start
				this.initLeafletMapContainer( this.ethnicDiversityDataValue.region );
				// Region Map Ends

				this.msmeStatus = this.ethnicDiversityDataValue.msmeStatus;
				this.totalActiveDirectorCount = this.ethnicDiversityDataValue.totalActiveDirectorCount;
				this.averageActiveDirectorCount = this.ethnicDiversityDataValue.averageActiveDirectorCount;
	...
	async initDoughnutChartContainer( chartData, chartFor ) {
		...
		for ( let dataKey in chartData ) {
			...
			if ( this.ethnicDiversityDataValue[ chartFor ].hasOwnProperty( dataKey ) && dataKey.split('_').length < 2 ) {
				chartLabels.push( this.commonService.camelCaseToSentence( dataKey.trim() ) );

				chartDataset.push( this.ethnicDiversityDataValue[ chartFor ][ dataKey ] );
				
				if ( chartFor == "riskChart" ) {
					backgroudColor.push(this.formatBgColorForPieChart( dataKey ));
					chartPercentage.push( this.ethnicDiversityDataValue[ chartFor ][ dataKey + '_percentage' ] );
				} else {
		...
		const onChartElementClick = ( event, elements, chart,  chartFor ) => {
			...
			if( chartdetails && elements && elements.length ) {
				...
				let navigateUrlParamObject = JSON.parse( JSON.stringify( this.ethnicDiversityDataObject.filterData ) );
	...
	async initLeafletMapContainer( currentYearData ) {
        ...
        instance.mapConfig.primaryMapId = `ethnicDiversityMapContainer`;
		instance.mapData = { currentYearData: currentYearData };
		instance.requiredData = { 
			thisPage: 'ethnicDiversity',
			globalFilterDataObject: this.ethnicDiversityDataObject
		};
		instance.itag = { icon: true, message: 'Categorises businesses by region in the UK, showing geographic distribution trends. Click on a region to explore business density and ownership patterns.'}
	...
	gotToSearchPage( chipDataValueTo?, chipDataValueFrom?, from? ) {
		...
		let obj = {
			...
			1000: ['Greater than 1000', 'greater']
		}
				...
				navigateUrlParamObject.push(
					{
						chip_group: "Number of Employees",
						chip_values: [ [+chipDataValueTo, +chipDataValueFrom] ],
						ageOperator: [obj[chipDataValueTo][1], 'fromEthinicityPage']
					}
				);
			}
		}
	...
	goToSearchForMisc( miscName, miscValue? ) {
		...
		navigateUrlParamObject = JSON.parse( JSON.stringify( this.ethnicDiversityDataObject.filterData ) );
	...
	customSort( event, ethnicTable, field) {
		...
		let tableDomElement = this.ethnicDiversityTable.el.nativeElement.children[0],
	...
	takeSnapshot( insightsChartViewContainerId ){
		this.sharedScreenshotService.snapshotForStatsInsight( insightsChartViewContainerId, 'DG_Ethnic_Diversity.jpeg' );
	}
